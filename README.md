...
2. 扫描产品SN，FeedingCheck返回OK后开始啮合。队列最多容纳两件未结束任务，同一SN在队列中不能再次进站；结束或全部终止后允许同一SN重新扫码。
...
5. TJSHEET必须恰好一行，Fi、fii、Fr均不能为NULL。成功读取有效数据后立即进入出站，不再复查文件变化。
6. 提交MES出站，明确返回PASS或FAIL均结束本次任务、释放队列位置；界面用两个固定卡片分别显示SN、扫码时间、进站/出站结果、MDB及数值。两件结束结果各自保留，空位优先接收新任务，没有空位时替换已结束的旧任务，未结束任务不会被覆盖。未知结果保留任务，人工核实后重试。
7. 点击“终止全部任务”会一起终止队列中的两件任务，没有单件终止按钮。
...
- 每次请求超时30秒，无自动网络重发。FeedingCheck失败或结果未知可重试；出站FAIL直接结束，只有出站结果未知才保留原请求供重试。“重试原任务”处理队列中最早可重试的任务，出站重试不重新读取或更换MDB。
...
- 新增简易出站日志默认位于EXE旁logs/checkout/YYYYMMDD.txt（可用CheckoutLogDirectory配置），每行JSON仅包含SN、MDB完整路径和出站SNInfo数据，不包含Token等接口元数据。每次发送前追加一行，同一SN多次加工及未知结果的人工重试分别追加，不覆盖旧记录。日志写入失败时暂缓发送，写入恢复后继续。
...
验证使用临时文件、模拟时钟和内存MES网关，不连接生产MES或PLC。覆盖两件队列、提前绑定、1秒等待、NG结束、重复扫码、并发请求、文件读取失败、文件变化不阻塞上传、日志写入失败、未知结果重试和全部终止。现场串口、真实MDB延迟写入顺序和实际MES仍需联调。
